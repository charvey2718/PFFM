...
int main()
{
	...
	std::vector<double> F1_history, D1_history, F2_history, D2_history, a_history, G_history;
	...
	if(restart_from > 0) // Load in starting condition from saved files
	{
		...
		while(std::getline(file, line))
		{
			...
			for(size_t i = 0; i < fields.size(); i++)
			{
				...
				else if(i == 6) G_history.push_back(val);
	...
	for(double D = Dinit; D <= Dend + Dinc/10; D += Dinc) // apply displacement incrementally
	{
		...
		while(true)
		{
            ...
            if(R_normalised < tol)
			{
				...
				{
					double x = nodes[node][0];
					double s = d(2*numNodes + node);
					if(s > 0.01) intact_length = x;
					else break;
				}
				...
				// Get Gc by compliance calibration - overwrite Gc history every time with latest best fit to all propagation data
				G_history = GetGc(D1_history, D2_history, F1_history, a_history, t, a_min);
				
				// save increment to CSV
				if(incr_count % save_freq == 0)
				{
					std::ofstream outfile(history_filename); // overwrite
					outfile << "inc,D1,F1,D2,F2,a,G,F1_theory,F2_theory,G_theory,a_theory" << std::endl;
...
std::vector<double> linspace(double start, double end, double initial_size, double growth_rate, double tol)
{
	...
	// std::cout << "adjusted_growth = " << adjusted_growth <<std::endl;
...
void AssembleLinearSolidMatrixSlow(Eigen::SparseMatrix<double>& K, Eigen::MatrixXd& k, int i, int j, int l, int m)
{
	K.coeffRef(2*i,     2*i    ) += k(0, 0);
	K.coeffRef(2*i,     2*i + 1) += k(0, 1);
	K.coeffRef(2*i,     2*j    ) += k(0, 2);
	K.coeffRef(2*i,     2*j + 1) += k(0, 3);
	K.coeffRef(2*i,     2*l    ) += k(0, 4);
	K.coeffRef(2*i,     2*l + 1) += k(0, 5);
	K.coeffRef(2*i,     2*m    ) += k(0, 6);
	K.coeffRef(2*i,     2*m + 1) += k(0, 7);
	
	K.coeffRef(2*i + 1, 2*i    ) += k(1, 0);
	K.coeffRef(2*i + 1, 2*i + 1) += k(1, 1);
	K.coeffRef(2*i + 1, 2*j    ) += k(1, 2);
	K.coeffRef(2*i + 1, 2*j + 1) += k(1, 3);
	K.coeffRef(2*i + 1, 2*l    ) += k(1, 4);
	K.coeffRef(2*i + 1, 2*l + 1) += k(1, 5);
	K.coeffRef(2*i + 1, 2*m    ) += k(1, 6);
	K.coeffRef(2*i + 1, 2*m + 1) += k(1, 7);
	
	K.coeffRef(2*j,     2*i    ) += k(2, 0);
	K.coeffRef(2*j,     2*i + 1) += k(2, 1);
	K.coeffRef(2*j,     2*j    ) += k(2, 2);
	K.coeffRef(2*j,     2*j + 1) += k(2, 3);
	K.coeffRef(2*j,     2*l    ) += k(2, 4);
	K.coeffRef(2*j,     2*l + 1) += k(2, 5);
	K.coeffRef(2*j,     2*m    ) += k(2, 6);
	K.coeffRef(2*j,     2*m + 1) += k(2, 7);
	
	K.coeffRef(2*j + 1, 2*i    ) += k(3, 0);
	K.coeffRef(2*j + 1, 2*i + 1) += k(3, 1);
	K.coeffRef(2*j + 1, 2*j    ) += k(3, 2);
	K.coeffRef(2*j + 1, 2*j + 1) += k(3, 3);
	K.coeffRef(2*j + 1, 2*l    ) += k(3, 4);
	K.coeffRef(2*j + 1, 2*l + 1) += k(3, 5);
	K.coeffRef(2*j + 1, 2*m    ) += k(3, 6);
	K.coeffRef(2*j + 1, 2*m + 1) += k(3, 7);

	K.coeffRef(2*l,     2*i    ) += k(4, 0);
	K.coeffRef(2*l,     2*i + 1) += k(4, 1);
	K.coeffRef(2*l,     2*j    ) += k(4, 2);
	K.coeffRef(2*l,     2*j + 1) += k(4, 3);
	K.coeffRef(2*l,     2*l    ) += k(4, 4);
	K.coeffRef(2*l,     2*l + 1) += k(4, 5);
	K.coeffRef(2*l,     2*m    ) += k(4, 6);
	K.coeffRef(2*l,     2*m + 1) += k(4, 7);
	
	K.coeffRef(2*l + 1, 2*i    ) += k(5, 0);
	K.coeffRef(2*l + 1, 2*i + 1) += k(5, 1);
	K.coeffRef(2*l + 1, 2*j    ) += k(5, 2);
	K.coeffRef(2*l + 1, 2*j + 1) += k(5, 3);
	K.coeffRef(2*l + 1, 2*l    ) += k(5, 4);
	K.coeffRef(2*l + 1, 2*l + 1) += k(5, 5);
	K.coeffRef(2*l + 1, 2*m    ) += k(5, 6);
	K.coeffRef(2*l + 1, 2*m + 1) += k(5, 7);
	
	K.coeffRef(2*m,     2*i    ) += k(6, 0);
	K.coeffRef(2*m,     2*i + 1) += k(6, 1);
	K.coeffRef(2*m,     2*j    ) += k(6, 2);
	K.coeffRef(2*m,     2*j + 1) += k(6, 3);
	K.coeffRef(2*m,     2*l    ) += k(6, 4);
	K.coeffRef(2*m,     2*l + 1) += k(6, 5);
	K.coeffRef(2*m,     2*m    ) += k(6, 6);
	K.coeffRef(2*m,     2*m + 1) += k(6, 7);
	
	K.coeffRef(2*m + 1, 2*i    ) += k(7, 0);
	K.coeffRef(2*m + 1, 2*i + 1) += k(7, 1);
	K.coeffRef(2*m + 1, 2*j    ) += k(7, 2);
	K.coeffRef(2*m + 1, 2*j + 1) += k(7, 3);
	K.coeffRef(2*m + 1, 2*l    ) += k(7, 4);
	K.coeffRef(2*m + 1, 2*l + 1) += k(7, 5);
	K.coeffRef(2*m + 1, 2*m    ) += k(7, 6);
	K.coeffRef(2*m + 1, 2*m + 1) += k(7, 7);
}

void AssembleLinearSolidMatrix(std::vector<Eigen::Triplet<double>>& triplets, Eigen::MatrixXd& k, int i, int j, int l, int m)
{
	int nodes[4] = {i, j, l, m};
...
void AssembleLinearPFFMMatrix(std::vector<Eigen::Triplet<double>>& triplets, Eigen::MatrixXd& k, int i, int j, int l, int m)
{
	int nodes[4] = {i, j, l, m};
...
void AssembleLinearPFFMVector(Eigen::VectorXd& F, Eigen::VectorXd& f, int i, int j, int l, int m)
{
	int nodes[4] = {i, j, l, m};
...
void AssembleQuadraticSolidMatrix(std::vector<Eigen::Triplet<double>>& triplets, Eigen::MatrixXd& k, int i, int j, int l, int m, int n, int o, int p, int q, int r)
{
...
void AssembleQuadraticPFFMMatrix(std::vector<Eigen::Triplet<double>>& triplets, Eigen::MatrixXd& k, int i, int j, int l, int m, int n, int o, int p, int q, int r)
{
	int nodes[9] = {i, j, l, m, n, o, p, q, r};
...
void AssembleQuadraticPFFMVector(Eigen::VectorXd& F, Eigen::VectorXd& f, int i, int j, int l, int m, int n, int o, int p, int q, int r)
{
	int nodes[9] = {i, j, l, m, n, o, p, q, r};
...
void AssembleAll(std::vector<std::vector<double>>& nodes, std::vector<std::vector<int>>& elements, Eigen::VectorXd& u, Eigen::VectorXd& s, std::vector<std::vector<double> >& H, double E_int, double E_bulk, double nu_int, double nu_bulk, double Gc_eff, double Gc_bulk, double t, double h2, double hi, double ls, Eigen::SparseMatrix<double>& K, Eigen::VectorXd& F, bool isPlaneStress, bool isQuadratic, int quadratureDegree)
{
...
void UpdateH(std::vector<std::vector<double>>& nodes, std::vector<std::vector<int>>& elements, Eigen::VectorXd& u, std::vector<std::vector<double> >& H, double E_int, double E_bulk, double nu_int, double nu_bulk, double h2, double hi, double ls, bool isPlaneStress, bool isQuadratic, int quadratureDegree)
{
	...
	for(int i = 0; i < elements.size(); ++i)
	{
		if(isQuadratic)
		{
			...
			std::vector<double> U_tensile = GetTensileElasticStrainEnergy_Quadratic(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7, x8, y8, x9, y9, E, nu, u_el, isPlaneStress, quadratureDegree);
		...
		else // linear
		{
			...
			std::vector<double> H_old = H[i];
			std::vector<double> U_tensile = GetTensileElasticStrainEnergy_Linear(x1, y1, x2, y2, x3, y3, x4, y4, E, nu, u_el, isPlaneStress, quadratureDegree);
...
std::vector<double> GetTensileElasticStrainEnergy_Linear(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double E, double nu, Eigen::VectorXd u, bool plane_stress, int quadratureDegree)
{
	// constitutive properties matrix
	Eigen::MatrixXd D(3, 3);
	double K;
...
std::vector<double> GetTensileElasticStrainEnergy_Quadratic(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double x5, double y5, double x6, double y6, double x7, double y7, double x8, double y8, double x9, double y9, double E, double nu, Eigen::VectorXd u, bool plane_stress, int quadratureDegree)
{
	// constitutive properties matrix
	Eigen::MatrixXd D(3, 3);
	double K;
...
void add_crack(std::vector<std::vector<double>>& nodes, std::vector<std::vector<int>>& elements, double xct, double yc, double tol)
{
	...
	// Step 1: Find and duplicate nodes along crack line
	...
	// Step 2: Update elements on one side of the crack
...
std::vector<double> GetGc(std::vector<double>& D1, std::vector<double>& D2, std::vector<double>& P, std::vector<double>& a, double t, double a_min)
{
	size_t size = 0;
	...
	for(size_t i = 0; i < a.size(); ++i)
	{
		if(a[i] > a_min)
		{
			double dC_da = n*std::exp(A)*std::pow(a[i], n - 1.);
			Gc.push_back(0.5*t*P[i]*P[i]*dC_da);
		}
		else
		{
			Gc.push_back(0);
...
void SaveHField(std::vector<std::vector<double>>& nodes, std::vector<std::vector<int>>& elements, std::vector<std::vector<double> >& H, bool isQuadratic, int quadratureDegree, std::string filename, int prec)
{
	Eigen::VectorXd xi_points;
	Eigen::VectorXd eta_points;
	Eigen::VectorXd weights;
